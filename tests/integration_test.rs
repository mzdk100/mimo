...
async fn test_basic_chat() {
    ...
    let response = client.chat(request).await.expect("Request should succeed");
...
async fn test_streaming_chat() {
    ...
    let request = ChatRequest::new(Model::MiMoV2Flash.as_str()).user("Count from 1 to 5.");
...
async fn test_tool_calling() {
    ...
    let tool = Tool::function("get_time", "Get the current time for a given timezone");
